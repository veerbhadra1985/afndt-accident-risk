...
ap = argparse.ArgumentParser(); ap.add_argument("--csv", required=True); ap.add_argument("--out", default="v2_results")
...
R["info"]["audit"] = {k: v for k, v in audit.items() if k != "states"}
ctx = year_contexts(df); S = Structures(ctx); R["info"]["n_contexts"] = int(len(ctx)); save()
